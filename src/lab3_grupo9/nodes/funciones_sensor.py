...
def likelihood_field_range_finder_model(instancia, mediciones, particula, campo):
    ...
    for i in mediciones:
        if i != instancia.zmax and not np.isnan(i) and angulo < FOV/2 and angulo > -FOV/2:
            ...
            x_real = particula.x # A diferencia de antes, no se transforma porque ya esta en metros
            y_real = (instancia.alto - particula.y - 1)
...
def likelihood_field(instancia, mapa : str):
    print("Entrando al likelihood_field")
    ...
    for y in range(alto): #Recorremos la imagen, para ver las posiciones con obstaculos
        for x in range(ancho):
            if img[y,x] == 0: #Ingresamos posiciones con obstaculos
                instancia.posiciones_ocupadas.append((y, x))
                instancia.espacios_no_disponibles.append((y * instancia.res, x * instancia.res))
            elif img[y,x] == 255: #Ingresamos posiciones desocupadas
                instancia.espacios_disponibles.append((y * instancia.res, x * instancia.res)) # Agregamos las posiciones libres
            else:
                instancia.espacios_no_disponibles.append((y * instancia.res, x * instancia.res))
    #Convertimos en arrays de numpy ambas listas
    posiciones_ocupadas = np.array(instancia.posiciones_ocupadas)
    instancia.get_logger().info("Terminamos el mapa :D") 
    try:
        print("Cargando mapa")
        instancia.field = np.load("likelihood_field.npy")
        instancia.field_ready = True
        ...
        np.save("likelihood_field.npy", instancia.field)
